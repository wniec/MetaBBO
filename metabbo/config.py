...
def get_config(args=None):
    ...
    parser.add_argument(
        "--upperbound", type=float, default=5, help="upperbound of search space"
    )
    parser.add_argument(
        "--difficulty",
        default="easy",
    ...
    parser.add_argument(
        "--test", default=None, action="store_true", help="switch to inference mode"
    )
    parser.add_argument(
        "--rollout", default=None, action="store_true", help="switch to rollout mode"
    )
    parser.add_argument(
        "--run_experiment",
        default=None,
        action="store_true",
        help="switch to run_experiment mode",
    )
    parser.add_argument(
        "--mgd_test", default=None, action="store_true", help="switch to mgd_test mode"
    )
    parser.add_argument(
        "--mte_test", default=None, action="store_true", help="switch to mte_test mode"
    )
    ...
    parser.add_argument(
        "--train_optimizer", default=None, help="optimizer for training"
    )
    parser.add_argument(
        "--agent_save_dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="interval epochs in drawing figures",
    )
    parser.add_argument(
        "--agent_for_plot_training",
        type=str,
        nargs="+",
        default=["RL_HPSDE_Agent"],
        help="learnable optimizer to compare",
    )
    parser.add_argument(
        "--n_checkpoint", type=int, default=20, help="number of training checkpoints"
    )
    parser.add_argument(
        "--resume_dir", type=str, help="directory to load previous checkpoint model"
    )
    ...
    parser.add_argument(
        "--agent",
        default=None,
        help="None: traditional optimizer, else Learnable optimizer",
    )
    parser.add_argument("--agent_load_dir", type=str, help="load your own agent model")
    parser.add_argument(
        "--optimizer", default=None, help="your own learnable or traditional optimizer"
    )
    parser.add_argument(
        "--agent_for_cp",
        type=str,
    ...
    # Rollout parameters
    parser.add_argument(
        "--agent_for_rollout", type=str, nargs="+", help="learnable agent for rollout"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--model_from", type=str, help="the model trained on source problem set"
    )
    parser.add_argument(
        "--model_to", type=str, help="the model trained on target problem set"
    )
    ...
    parser.add_argument(
        "--pre_train_rollout",
        type=str,
        help="path of pre-train models rollout result .pkl file",
    )
    parser.add_argument(
        "--scratch_rollout",
        type=str,
        help="path of scratch models rollout result .pkl file",
    )
    ...
    if config.run_experiment and len(config.agent_for_cp) >= 1:
        assert config.agent_load_dir is not None, (
            "Option --agent_load_dir must be given since you specified option --agent_for_cp."
        )
